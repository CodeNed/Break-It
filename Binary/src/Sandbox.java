...
public class Sandbox {
	...
	static class Level {
		...
		public void save(String path) {
			...
			RCObject levelObject = new RCObject("LevelData");
			levelObject.addString(RCString.Create("name", name));
			levelObject.addString(RCString.Create("path", this.path));
			levelObject.addField(RCField.Integer("width", width));
			levelObject.addField(RCField.Integer("height", height));
			levelObject.addField(RCField.Integer("entityCount", entities.size()));
			database.addObject(levelObject);
			for (int i = 0; i < entities.size(); i++) {
				Entity e = entities.get(i);

				RCObject object = new RCObject("E" + i);
				byte type = 0;
				if (e instanceof Player) type = 1;
				object.addField(RCField.Byte("type", type));
				object.addField(RCField.Integer("arrayIndex", i));
				e.serialize(object);
		...
		public static Level load(String path) {
			...
			for (int i = 0; i < entityCount; i++) {
				int offset = 1 + i;
				RCObject entity = database.objects.get(offset);
			}

			for (int i = 0; i < entityCount; i++) {
				RCObject object = database.findObject("E" + i);
				Entity e;

				if (object.findField("type").getByte() == 1)
					e = Player.deserialize(object);
				else
					e = Entity.deserialize(object);
				result.add(e);
			}
			return result;
		}
	...
	public void play() {
		{
			...
			Player player = new Player("Noah", 40, 28);
